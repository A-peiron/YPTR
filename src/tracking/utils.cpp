                          ...
                          bool extend_cost, float cost_limit, bool return_cost)
{
    ...
    else
    {
        if (!extend_cost)
        {
            ...
            // system("pause"); // 移除Windows特定的pause命令
            std::cerr << "Error: extend_cost must be True for this operation" << std::endl;
            exit(1);  // 使用标准错误码
        }
    }
    ...
    if (ret != 0)
    {
        ...
        // system("pause"); // 移除Windows特定的pause命令
        std::cerr << "Error: LAPJV calculation failed with error code: " << ret << std::endl;
        exit(1);  // 使用标准错误码
    }
